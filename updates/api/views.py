...
class UpdateModelDetailAPIView(HttpResponseMixin, CSRFExemptMixin, View):
    ...
    def put(self, request, id, *args, **kwargs):
        ...
        for key, value in passed_data.items():
            data[key] = value
        # problem end
        form = UpdateModelFrom(data, instance=obj)
        if form.is_valid():
    ...
    def delete(self, request, id, *args, **kwargs):
        ...
        error_data = json.dumps({"message": "Could not delete item. Please try again later or know your developer."})
        return self.render_to_response(error_data, status=400)


# One endpoint '/api/updates/' rule the model
...
class UpdateModelListAPIView(HttpResponseMixin, CSRFExemptMixin, View):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if not passed_id:
            error_data = json.dumps({"id": "ID Not Found!"})
            return self.render_to_response(error_data, status=400)
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if not passed_id:
            error_data = json.dumps({"id": "Deleted ID Not Found!"})
            return self.render_to_response(error_data, status=400)
